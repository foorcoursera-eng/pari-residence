/* ==========================================================================
   ...
   Копия из pari-residence (lib/crm/telegram.js) для нового сайта pari-redesign:
   в сообщение добавлена строка «Сайт», чтобы при общем боте было видно, откуда заявка.
   ========================================================================== */
...
const NAMES = {
  ru: { title: 'Новая заявка — PARI Residence', name: 'Имя', phone: 'Телефон', page: 'Страница',
        ...
        rooms: 'Интересует', flat: 'Квартира',
        device: 'Устройство', lang: 'Язык', site: 'Сайт' },
};
...
async function sendLead(lead) {
  ...
  if (lead.site) { lines.splice(1, 0, `${L.site}: ${lead.site}`); }
  if (lead.rooms) { lines.splice(3, 0, `${L.rooms}: ${lead.rooms}`); }
  if (lead.flat) { lines.splice(3, 0, `${L.flat}: ${lead.flat}`); }
