...
/* Адрес сайта. Домен подключён 30.08.2026: A-запись корня и CNAME для www
   стоят в DNS UZCLOUD, Vercel держит основным адресом www и переадресовывает
   на него корень (308). Здесь указан ровно тот адрес, которым сайт отвечает
   в конце цепочки: canonical, карта сайта и og:url обязаны совпадать с ним,
   иначе поисковик считает основным другой хост.
   Переменная окружения SITE_ORIGIN по-прежнему перебивает это значение —
   пригодится, если основным сделают адрес без www. */
const ORIGIN = process.env.SITE_ORIGIN || 'https://www.pari-residence.uz';
